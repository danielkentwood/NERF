classdef HDBSCAN < handle
    ...
    %                       (see below) greater than 'outlierThresh' is assigned
...
%                 h = scatter( self.data(:,dims(1)),self.data(:,dims(2)),'.' );
                cols = self.cluster_colors;
                h = gscatter( self.data(:,dims(1)), self.data(:,dims(2)), self.labels, cols(1:length(unique(self.labels)),:));
            end
...
%             end
            ...
            % change appearance
            title( 'Clustered data','color','k' );
            set( h(1).Parent,'tickdir','out','box','off');
        end
